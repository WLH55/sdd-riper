...
| Debug 模式 | 日志驱动排查 | `references/modules.md` 中 Debug 段 |

## 参考

- `references/sdd-riper-one-protocol.md` - 完整协议定义
- `references/spec-template.md` - Spec 模板（含状态块）
...
4. **用户批准**：审查 spec 和 plan，说 "Plan Approved" 授权执行
5. **执行 + 回写**：AI 按 plan 执行代码变更，完成后自动回写 spec

> 中断后恢复：新会话输入 `/sdd:resume` 即可从上次断点继续。
