...
```text
进入 review，对照 spec 给出覆盖率和偏差；若失败回到 plan。
```
...
- 交叉比对日志证据 × Spec 预期 × 代码逻辑，输出根因分析报告：
  - **症状**：日志中审批回调已触发，但 `triggerGrant` 未执行。
  - **预期行为**：审批通过（approved_state）应触发自动授权。
  - **根因候选**：`ApprovalCallbackHandler` 中灰度判断条件未命中（灰度 key 未配置）。
